...
class InstagramPoster:
    ...
    def post_image(self, image_url, caption):
        logger.info(f"Starting Instagram post with image URL: {image_url}, caption length: {len(caption) if caption else 0}")
        
        if not self.ig_user_id or not self.access_token:
            error_msg = f"Missing Instagram credentials - User ID: {bool(self.ig_user_id)}, Access Token: {bool(self.access_token)}"
        ...
        try:
            ...
            logger.debug(f"Media creation payload: {{'image_url': '{image_url}', 'caption_length': {len(caption)}, 'access_token': '[HIDDEN]'}}")
            
            response = requests.post(create_url, data=payload, timeout=30)
            
            logger.info(f"Instagram media creation response status: {response.status_code}")
            logger.debug(f"Instagram creation response headers: {dict(response.headers)}")
            ...
            logger.info(f"Instagram media publish response status: {publish_response.status_code}")
            ...
            error_msg = f"Instagram HTTP error: {response.status_code if 'response' in locals() else publish_response.status_code if 'publish_response' in locals() else 'Unknown'} - {response_text}"
            logger.error(error_msg)
            raise Exception(error_msg)
    ...
    def post_image_fake(self, image_path, caption):
        logger.info(f"FAKE Instagram post - Image: {image_path}, Caption length: {len(caption) if caption else 0}")
