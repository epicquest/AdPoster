...
class TwitterPoster:
    def __init__(
        ...
        logger.info(
            f"TwitterPoster initializing with credentials - API Key: {bool(api_key)}, "
            f"Access Token: {bool(access_token)}"
        )

        if not all([access_token, access_token_secret, api_key, api_key_secret]):
            error_msg = (
                f"Missing Twitter credentials - API Key: {bool(api_key)}, "
                f"API Secret: {bool(api_key_secret)}, "
                f"Access Token: {bool(access_token)}, "
                f"Access Secret: {bool(access_token_secret)}"
            )
    ...
    def _post_image_and_tweet(
        self, image_path, message, reply_message=None, app_url: str | None = None
    ):
        logger.info(
            f"Starting Twitter image post - Image: {image_path}, "
    ...
    def post_tweet_with_image(self, message, media_id=None):
        ...
        """
        logger.info(
            f"Creating Twitter tweet with image - Message length: "
            f"{len(message) if message else 0}, Has media: {bool(media_id)}"
        )
    ...
    def reply_to_tweet(self, tweet_id, message):
        ...
        """
        logger.info(
            f"Creating Twitter reply to tweet {tweet_id}, "
    ...
    def post_text_and_link(self, message: str, app_url: Optional[str] = None):
        ...
        """
        logger.info(
            f"Creating Twitter text post - Message length: {len(message)}, "
            f"Has app URL: {bool(app_url)}"
        )
        ...
        try:
            ...
            if app_url:
                ...
                logger.info(
                    f"Added app URL to Twitter message, "
                    f"final length: {len(full_message)}"
                )
    ...
    def _post_tweet(self, message: str):
        ...
        """
        logger.info(
            f"Creating Twitter text-only tweet - Message length: {len(message)}"
        )
